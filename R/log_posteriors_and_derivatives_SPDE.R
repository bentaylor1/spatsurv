...
logPosterior_SPDE <- function(surv,X,beta,omega,eta,gamma,priors,cov.model,u,control,gradient=FALSE,hessian=FALSE){
    ...
    if(gradient){
        ...
        if(censoringtype=="right"){
            ...
            dP_dgamma <- as.numeric(Matrix::solve(U,bitsnbobs)) #cholsigma%*%bitsnbobs #as.vector(Re((1/(control$Mext*control$Next))*fft(invrootQeigs*fft(bitsnbobs,inverse=TRUE))))
    ...
    if(hessian){
        ...
        if(censoringtype=="right"){
            ...
            hess_gamma <- as.numeric(Matrix::solve(prec,bitsnbobs)) #sigma%*%bitsnbobs #as.vector(Re((1/(control$Mext*control$Next))*fft(covbase*fft(bitsnbobs,inverse=TRUE))))
